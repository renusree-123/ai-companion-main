 ...
 *
 * Responsibilities:
 *   1. Provider selection (live Groq, or the offline provider)
...
export function getPrimaryProvider(): AiProvider {
  ...
  if (config.AI_PROVIDER === "offline" || !hasLiveProvider()) {
    ...
    primary = new GroqProvider(config.GROQ_API_KEY!, config.AI_MODEL);
  }
  return primary;
}
...
function getFallbackProvider(): AiProvider | null {
  ...
  fallback ??= new GroqProvider(config.GROQ_API_KEY!, config.AI_FALLBACK_MODEL);
  return fallback;
}
...
/**
 * Whether the offline provider may answer for a *failing live provider*.
 *
 * When no key is configured the offline provider is already the primary, so
 * this only governs the case where Gemini is configured and erroring. There
 * the default is no: extractive offline output returned under a configured
 * model looks like a working feature producing bad answers, and it hides the
 * error that would have explained it. `AI_OFFLINE_FALLBACK=true` restores
 * degrade-over-fail for deployments that prefer it.
 */
function offlineFallbackAllowed(): boolean {
  if (!hasLiveProvider()) return true;
  return env().AI_OFFLINE_FALLBACK === "true";
}
...
async function execute<T>(
  ...
  if (options.allowOfflineFallback && offlineFallbackAllowed() && !chain.includes(offline)) {
    chain.push(offline);
  }
  ...
  logger.error("ai_all_providers_failed", {
    traceId: meta.traceId,
    feature: meta.feature,
    chain: chain.map((provider) => `${provider.id}:${provider.model}`),
    offlineFallback: offlineFallbackAllowed(),
