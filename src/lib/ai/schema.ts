...
function convertInner(schema: ZodTypeAny): Record<string, unknown> {
  ...
  switch (def.typeName) {
    ...
    // A preprocess/transform wrapper is a *validation* concern only. The model
    // still needs to see the constraints of the schema underneath it, so the
    // wrapper is transparent here.
    case z.ZodFirstPartyTypeKind.ZodEffects:
      return convert((def as any).schema);
