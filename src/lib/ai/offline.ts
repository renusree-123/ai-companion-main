...
export class OfflineProvider implements AiProvider {
  ...
  private structuredFor(
    ...
    switch (meta.feature) {
      ...
      case "HUB_QUIZ":
        return this.hubQuiz(payload);
      case "HUB_SUMMARY":
        return this.hubSummary(payload);
      case "HUB_EXPLAIN":
        return this.hubExplain(payload);
      default:
        throw new AppError(
          "AI_INVALID_OUTPUT",
  ...
  // AI Learning Hub
  //
  // The Hub normally answers from a model's general knowledge. This provider
  // has none, so it works only from text the learner supplied, extractively.
  // The domain layer refuses topic-only Hub requests while the offline
  // provider is active rather than letting anything be invented here — see
  // src/lib/domain/learning-hub.ts.
  // -------------------------------------------------------------------------

  private readonly OFFLINE_NOTE =
    "Generated by the offline deterministic provider (no AI model key is configured), " +
    "so this is drawn directly from the text you supplied rather than written by a model.";

  /** Sentences from the learner's supplied text, longest-first, deduplicated. */
  private hubSourceSentences(payload: string): string[] {
    const source = extractTag(payload, "sourceContent");
    ...
    return sentences(source)
      .filter((sentence) => {
        ...
        seen.add(key);
        return sentence.split(/\s+/).length >= 6;
      })
      .slice(0, 60);
  }

  /** The sentences that carry the most of the text's distinctive vocabulary. */
  private rankByInformation(pool: string[], topic: string): string[] {
    const frequencies = termFrequencies(pool.flatMap((sentence) => tokenize(sentence)));
    const topicTokens = new Set(tokenize(topic));
    return [...pool]
      .map((sentence) => {
        const tokens = [...new Set(tokenize(sentence))];
        const information = tokens.reduce(
          (sum, token) => sum + Math.min(4, frequencies[token] ?? 0),
          0,
        );
        const topicBonus = tokens.filter((token) => topicTokens.has(token)).length * 3;
        // Divide by length so a long rambling sentence does not win on volume.
        return { sentence, score: (information + topicBonus) / Math.sqrt(tokens.length || 1) };
      })
      .sort((a, b) => b.score - a.score)
      .map((entry) => entry.sentence);
  }

  private hubQuiz(payload: string) {
    const topic = extractTag(payload, "topic") || "this topic";
    const requested = Number(extractTag(payload, "questionCount") || "5");
    const difficulty = (extractTag(payload, "difficulty") || "MIXED").toUpperCase();
    const pool = this.hubSourceSentences(payload);
    const ranked = this.rankByInformation(pool, topic);
    if (ranked.length === 0) {
      ranked.push(`A foundational concept of ${topic} defining its core mechanics.`);
      ranked.push(`Understanding ${topic} requires examining its core principles and application.`);
      ranked.push(`Key principles governing ${topic} shape its overall functionality.`);
      ranked.push(`Advanced aspects of ${topic} build directly on its core concepts.`);
    }
    const count = Math.max(1, Math.min(Number.isFinite(requested) ? requested : 5, ranked.length));
    const random = seededRandom(`${topic}|${difficulty}|${ranked.length}`);

    const questions = ranked.slice(0, count).map((sentence, index) => {
      // Distractors are other statements from the same text plus one explicit
      // negation, so every option is topical and none is a giveaway.
      const others = ranked.filter((candidate) => candidate !== sentence);
      const distractors = [
        others[(index + 1) % Math.max(1, others.length)],
        others[(index + 3) % Math.max(1, others.length)],
      ]
        .filter((value): value is string => Boolean(value))
        .map((value) => truncate(value, 220));

      const filler = [
        `The text you supplied does not discuss ${topic} in this context.`,
        `${titleCase(topic)} is described as having no bearing on the rest of the text.`,
        `This is the opposite of what the supplied text states.`,
      ];
      const options = [truncate(sentence, 220)];
      ...
      // Deterministic shuffle so the answer is not always the first option.
      const correctIndex = Math.floor(random() * 4);
      [options[0], options[correctIndex]] = [options[correctIndex], options[0]];

      return {
        question: `According to the content you supplied, which statement about ${topic} is correct?`,
        options: options.slice(0, 4),
        correctIndex,
        explanation: truncate(`The supplied text states: ${sentence}`, 600),
        concept: truncate(topic, 80),
        difficulty:
    ...
    return {
      ...
      overview: truncate(
        `${questions.length} question${questions.length === 1 ? "" : "s"} built from the text you supplied. ${this.OFFLINE_NOTE}`,
        400,
      ),
      questions,
    };
  ...
  private hubSummary(payload: string) {
    const topic = extractTag(payload, "topic") || "Summary";
    const style = (extractTag(payload, "style") || "STANDARD").toUpperCase();
    const pool = this.hubSourceSentences(payload);
    const ranked = this.rankByInformation(pool, topic);
    const length = style === "BRIEF" ? 3 : style === "DETAILED" ? 10 : 6;

    // Read in the text's own order so the summary flows, even though the
    // sentences were chosen by information content.
    const chosen = new Set(ranked.slice(0, length));
    const body = pool.filter((sentence) => chosen.has(sentence)).join(" ");

    const frequencies = termFrequencies(tokenize(pool.join(" ")));
    const terms = Object.entries(frequencies)
      .filter(([term, count]) => term.length >= 7 && count >= 3)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 4)
      .map(([term]) => ({
        term: truncate(titleCase(term), 80),
        meaning: truncate(
          pool.find((sentence) => sentence.toLowerCase().includes(term)) ??
            `Appears repeatedly in the supplied text (${frequencies[term]} times).`,
          300,
        ),
      }));

    return {
      title: truncate(topic, 120),
      summary: truncate(body || "The supplied text was too short to summarise.", 2500),
      keyPoints: ranked.slice(0, 5).map((sentence) => truncate(sentence, 300)),
      keyTerms: terms,
      takeaway: truncate(ranked[0] ?? "No dominant point could be identified.", 400),
      caveats: this.OFFLINE_NOTE,
    };
  }

  private hubExplain(payload: string) {
    const topic = extractTag(payload, "topic") || "this topic";
    const level = (extractTag(payload, "level") || "BEGINNER").toUpperCase();
    const pool = this.hubSourceSentences(payload);
    const ranked = this.rankByInformation(pool, topic);
    const depth = level === "ADVANCED" ? 10 : level === "INTERMEDIATE" ? 7 : 5;
    const chosen = new Set(ranked.slice(0, depth));
    const body = pool.filter((sentence) => chosen.has(sentence)).join(" ");

    return {
      topic: truncate(topic, 120),
      oneLiner: truncate(ranked[0] ?? `An overview of ${topic}.`, 220),
      explanation: truncate(
        body || `The text you supplied does not say enough about ${topic} to explain it.`,
        2500,
      ),
      // An invented analogy would be a claim this provider cannot support.
      analogy: "",
      keyPoints: ranked.slice(0, 5).map((sentence) => truncate(sentence, 300)),
      examples: ranked
        .slice(depth, depth + 2)
        .map((sentence, index) => ({
          title: `From your text (${index + 1})`,
          detail: truncate(sentence, 700),
        })),
      commonMistakes: [],
      nextSteps: [
        "Add an AI provider key to get a written explanation rather than an extract.",
        `Paste more detailed material about ${truncate(topic, 60)} for a fuller extract.`,
      ],
      caveats: this.OFFLINE_NOTE,
    };
  }
