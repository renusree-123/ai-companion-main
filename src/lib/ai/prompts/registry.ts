...
export interface StructuredPrompt<Input, Output> {
  ...
  /**
   * Input type is left open: a schema may normalise what the model returned
   * (clamping an over-long string, say) before validating it, which makes its
   * input type differ from its output type. Only the output side is a
   * contract, and that stays exact.
   */
