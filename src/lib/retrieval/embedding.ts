 ...
 *
 *   - Gemini does serve an embedding endpoint, so this is now a deliberate
 *     choice rather than a constraint: a hosted embedding call would put a
 *     network round trip, a rate limit and a failure mode on the critical path
 *     of document processing, and would need a re-index to adopt. It is the
 *     obvious next upgrade, not a gap — see docs/LIMITATIONS.md.
