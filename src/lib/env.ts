 ...
 * AUTH_SECRET (which has a dev-only fallback). Notably GEMINI_API_KEY is
 ...
 * The key is read here, on the server, and never reaches the browser — no
 * NEXT_PUBLIC_ prefix, and no AI call is made from client code.
 */
const schema = z.object({
   ...
   * offline provider.
   */
  AI_OFFLINE_FALLBACK: z.enum(["true", "false"]).default("false"),
