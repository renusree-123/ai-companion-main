...
export async function generateNextQuestion(params: {
  ...
  if (finalEvidence.length === 0) {
    finalEvidence = [
      {
        chunkId: `concept-topic-${selection.concept.conceptId}`,
        sourceLabel: selection.concept.name,
        text: `Core principles, definitions, and key topics regarding ${selection.concept.name}.`,
      },
    ];
  }
    ...
    return await db.quizQuestion.create({
      data: {
        ...
        sources: JSON.stringify(
          finalEvidence.map((e) => ({ label: e.sourceLabel, chunkId: e.chunkId })),
        ),
