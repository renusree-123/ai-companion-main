 ...
 * server.
 *
 * Provider errors are quoted verbatim in development because the provider's own
 * wording is the diagnosis. That is only safe if the quoting is filtered: SDK
 * errors sometimes carry the request URL, and a Gemini request URL carries the
...
/** Applies `redactSecrets` to every string inside an arbitrary structure. */
