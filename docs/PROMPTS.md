...
| 11 | Learning Hub quiz generation | `hub.quiz` | 1.0.0 |
| 12 | Learning Hub summarisation | `hub.summary` | 1.0.0 |
| 13 | Learning Hub topic explainer | `hub.explain` | 1.0.0 |
...
- **The Learning Hub prompts trade grounding for honesty rules.** They are the
  only prompts in the product that answer from general model knowledge, because
  the learner arrives with a topic and no documents. `GROUNDING_CONTRACT` cannot
  apply, so a third shared fragment (`HUB_HONESTY`) replaces it: no fabricated
  statistics, dates, citations or studies; contested or possibly-stale ground
  flagged in a dedicated `caveats` field; and pasted `<sourceContent>` takes
  priority over the model's own knowledge when it is supplied. The UI labels
  every Hub result as general knowledge and points at the Tutor for anything
  that needs citations.
- **An absent source is rendered outside the data tag.** When no content is
  pasted, the "answer from general knowledge" note sits *outside*
  `<sourceContent>` rather than inside it. Inside, an empty request looks like a
  request with one sentence of source material — and the extractive offline
  provider would quote that placeholder back as if it were the learner's own
  text. This was caught by a check that asserted the offline provider refuses a
  topic-only quiz; it was silently generating one instead.
...
All three work with no API key. Add `GEMINI_API_KEY` to run the same
