...
| | |
| --- | --- |
| **Primary** | Google Gemini, `AI_MODEL` (default `gemini-2.5-flash`; `gemini-3.6-flash` and other 3.x models are supported) |
| **Fallback tier** | `AI_FALLBACK_MODEL` (default `gemini-2.5-flash-lite`) |
| **Last resort** | Built-in deterministic offline provider |
| **SDK** | `@google/genai`, the official Google Gen AI SDK |
| **Credential** | `GEMINI_API_KEY`, read server-side only — no `NEXT_PUBLIC_` exposure, and no AI call originates in client code |
| **Abstraction** | `src/lib/ai/types.ts` — application code never imports the vendor SDK |

Both configured models are available on the Gemini free tier, so the whole
product runs at no cost within the published rate limits.

Requests are non-streaming for structured work and streaming for the tutor.
Structured output uses Gemini's `responseSchema` with a JSON response type, so
decoding is constrained to the schema; the result is re-validated with Zod
because the application, not the model, owns the contract.

**Thinking is configured per model family**, and the two families are not
interchangeable:

| Family | Parameter | Notes |
| --- | --- | --- |
| Gemini 2.x and earlier | `thinkingBudget` (tokens) | `0` disables thinking |
| Gemini 3.x | `thinkingLevel` (`minimal`/`low`/`medium`/`high`) | Cannot be disabled; `low` is the default |
| Unrecognised | none sent | Omitting is always valid; guessing is not |

Sending the wrong parameter — or both at once — is a 400 `INVALID_ARGUMENT`
before the model sees the prompt, which fails every AI feature simultaneously.
`thinkingConfigFor()` picks by model name, and a 400 that names a parameter we
can drop is retried once without it, logged at warn level.

Thought tokens are drawn from `maxOutputTokens`, so headroom is added on top of
the caller's allowance rather than taken out of it. Without it a structured
prompt can spend its whole allowance reasoning and return an empty candidate —
a failure that reads like a schema bug.
...
| Hub quiz generator | `hub.quiz@1.0.0` | Structured | MCQ quiz on any topic, with answer key and per-question explanations |
| Hub summariser | `hub.summary@1.0.0` | Structured | Summary, key points, key terms and takeaway for pasted content or a topic |
| Hub explainer | `hub.explain@1.0.0` | Structured | Plain explanation with analogy, worked examples, common mistakes, next steps |

The three `hub.*` prompts are the only place the product answers from general
model knowledge rather than from the learner's own materials — the Learning Hub
is topic-first and has no documents to ground in. They are held to explicit
honesty rules instead (no invented figures, citations or studies; uncertainty
surfaced in a `caveats` field), every result is labelled as general knowledge in
the UI, and none of their output is allowed to move a mastery estimate.
