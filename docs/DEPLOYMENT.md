...
| `GEMINI_API_KEY` | no | — | Server-side only. Without it, the offline provider runs |
| `AI_PROVIDER` | no | `auto` | `auto` \| `gemini` \| `offline` |
| `AI_MODEL` | no | `gemini-2.5-flash` | Free tier |
| `AI_FALLBACK_MODEL` | no | `gemini-2.5-flash-lite` | Free tier; tried once before the offline provider |
| `AI_THINKING_BUDGET` | no | `0` | **Gemini 2.x only.** Thinking tokens: `0` off, `-1` dynamic |
| `AI_THINKING_LEVEL` | no | `low` | **Gemini 3.x only.** `minimal` \| `low` \| `medium` \| `high` |
| `AI_OFFLINE_FALLBACK` | no | `false` | `true` lets the offline provider answer when Gemini fails |

**Thinking parameters are not interchangeable.** Gemini 2.x takes
`thinkingBudget`; Gemini 3.x takes `thinkingLevel` and rejects a request
carrying both. The provider picks by model name, so normally neither variable
needs setting — they only tune a family that is already selected correctly. A
model the code does not recognise is sent no thinking parameter at all, which
is always valid.

**`AI_OFFLINE_FALLBACK` defaults to off deliberately.** With a key configured,
a Gemini failure should surface as an error rather than be answered by the
extractive offline provider: offline output is structurally valid, so masking
the failure turns a misconfiguration into mysteriously poor answers with no
error anywhere. Set it to `true` if you would rather degrade than fail.
