...
- **The AI Learning Hub is ungrounded by design, and that is a real limit.**
  Hub answers come from general model knowledge, so they carry no citations and
  cannot be verified against anything. The prompts are held to explicit honesty
  rules and every result is labelled in the UI, but a confident wrong answer is
  possible in a way it is not in the Tutor, where the relevance floor forces a
  refusal instead. Anything that matters should go through a Project.
- **Hub results are not revisable in place.** History lists past items and the
  API can reopen one, but the UI does not yet re-render a stored quiz for a
  retake — regenerating is the current path.
- **Free-tier rate limits are the practical ceiling.** `gemini-2.5-flash`
  allows roughly 10 requests a minute and 250 a day on the free tier. A quiz,
  a summary and a tutor answer are one request each, so ordinary study use is
  comfortable — but a demo that hammers the Hub, or several evaluators sharing
  one key, will hit a 429. The router surfaces it as a rate-limit message and
  falls back to `gemini-2.5-flash-lite`, which has a higher daily allowance.
- **Retrieval still uses local hashed embeddings, not Gemini's embedding API.**
  Now that the provider serves embeddings, this is a deliberate deferral rather
  than a constraint: adopting them means a network hop in document processing
  and a full re-index. It is the first thing I would change next.
- **The Hub degrades sharply without an API key.** The offline provider is
  extractive, so it can summarise or quiz pasted text but refuses a topic-only
  request rather than inventing one. That is the honest behaviour, but it means
  the credential-free demo shows less of this feature than of the rest. With
  `GEMINI_API_KEY` set, topic-only generation works for all three tools.
