...
**Why not a hosted embedding model.** Gemini does serve an embeddings
...
## Gemini 2.5 Flash behind a provider interface, with a deterministic offline fallback

**Why Gemini 2.5 Flash.** The product needs to be runnable by anyone reviewing
it, and Gemini's free tier makes that true without a funded account:
`gemini-2.5-flash` and `gemini-2.5-flash-lite` are both free-tier models, and
the latter is configured as the fallback tier. Flash is strong enough for the
two demanding tasks here — grounded synthesis with correct refusal, and
assessment questions whose distractors are plausible — and its constrained
JSON decoding suits a product where most calls are structured.

**What the swap cost.** Changing provider touched four files:
`src/lib/ai/gemini.ts` (new), the router's two constructor calls, the env
schema and the price table. Nothing else moved. That is the return on having
defined `AiProvider` up front rather than calling an SDK from the domain layer.
...
**Model families are a compatibility surface, not just a name.** Gemini 2.x
and 3.x take different thinking parameters, and sending the wrong one is a hard
400 rather than an ignored field — so a model-name change alone can break every
AI feature at once. The provider derives the parameter from the major version
rather than from an allow-list of model names, which would go stale the week
after it was written, and falls back to sending nothing for a model it does not
recognise. A 400 that explicitly names a droppable parameter is retried once
without it: per-model rules change faster than a pinned constant can track, and
thinking settings are an optimisation rather than semantics.

**Implementation note.** Structured generation uses `responseSchema` with
`responseMimeType: "application/json"`, so decoding is constrained to the
schema. Thinking is disabled by default: on the 2.5 family thinking tokens come
out of `maxOutputTokens`, so a 1024-token structured request can spend its
whole budget reasoning and return nothing — a failure mode that presents as a
schema error. `AI_THINKING_BUDGET` re-enables it and is added on top of the
caller's allowance. `temperature` is not sent; prompt design carries the
behaviour.
