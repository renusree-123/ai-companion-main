...
event: done
data: {"messageId":"…","grounding":"GROUNDED","citations":[…],"latencyMs":1840,"model":"gemini-2.5-flash"}
...
Topic-first tools that need no Space, Project or upload. Everything here is
scoped to the signed-in user; nothing reads or writes project state.

| Method | Path | Body | Notes |
| --- | --- | --- | --- |
| `POST` | `/api/learning-hub/quiz` | `{ topic, questionCount?: 1–15, difficulty?: EASY\|MEDIUM\|HARD\|MIXED, sourceContent?: ≤20k chars }` | Generates an MCQ quiz. **The answer key is withheld** — options only |
| `POST` | `/api/learning-hub/quiz/:artifactId/submit` | `{ answers: [{ index, selectedIndex \| null }] }` | Grades server-side against the stored key; returns score, per-question correctness and explanations |
| `POST` | `/api/learning-hub/summary` | `{ topic, sourceContent?, style?: BRIEF\|STANDARD\|DETAILED }` | Summarises pasted content, or the topic itself |
| `POST` | `/api/learning-hub/explain` | `{ topic, level?: BEGINNER\|INTERMEDIATE\|ADVANCED, sourceContent? }` | Explanation with analogy, examples, common mistakes, next steps |
| `GET` | `/api/learning-hub/history` | `?kind=QUIZ\|SUMMARY\|EXPLANATION&limit=1–50` | The user's recent Hub output |
| `GET` | `/api/learning-hub/items/:artifactId` | — | Reopens one stored result. A quiz reveals its answer key only after submission |

`POST /api/learning-hub/quiz` returns options without the answer:
...
- **Unanswered is wrong.** An omitted `index` grades as incorrect, so the score
  means "how much of this did you know".
- **Re-submitting is safe.** It overwrites the same row rather than creating a
  second attempt, and the completion activity is recorded once.
- **Hub output never reaches mastery.** These results are stored in
  `HubArtifact`, not `Quiz`, because nothing here is grounded in the learner's
  own materials — letting it move a mastery estimate would corrupt the evidence
  those estimates are built from.
- **Offline provider.** With no `GEMINI_API_KEY`, the Hub answers only from
  `sourceContent` and returns `409 CONFLICT` for a topic-only request rather
  than inventing a topic it knows nothing about.
