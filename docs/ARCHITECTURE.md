...
┌───────────────────────────────▼──────────────────────────────────────────┐
│  EXTERNAL                                                                 │
│  Google Gemini API (gemini-2.5-flash) — text, structured output,          │
│  streaming, native PDF understanding. Falls back to the offline provider. │
└──────────────────────────────────────────────────────────────────────────┘
```

Nothing above the data layer talks to Gemini directly. Application code
...
User ──┬── Space ──── Project ──┬── Material ──── MaterialChunk ──┬── ConceptChunk
       ...
       ├── ActivityEvent
       ├── HubArtifact           (AI Learning Hub — user-scoped, no project)
       └── UserDailyStat
...
- **`HubArtifact` deliberately hangs off `User`, not `Project`.** The AI
  Learning Hub is topic-first: its output is not grounded in the learner's
  materials, so storing it in `Quiz` would let ungrounded answers move mastery
  and pollute the evidence growth analysis is built from. A separate table means
  the Hub cannot affect any existing analytic by construction, rather than by
  remembering to filter it out everywhere.
...
domain service
   └─ runTextPrompt / runStructuredPrompt(prompt, input, ctx)
     ...
     AiProvider
        ...
        │     text · structured (responseSchema + Zod) · streaming · PDF vision
        └─ OfflineProvider     deterministic, extractive, no credentials
```

Structured output sets **`responseMimeType: application/json` plus a
`responseSchema`**, then re-validates with Zod. Constrained decoding makes the
output schema-valid at the API level; the Zod pass exists because the
*application*, not the model, owns the contract.

Gemini's schema dialect is an OpenAPI subset rather than JSON Schema, so
`toGeminiSchema` converts the schema derived from Zod: types become upper-case,
`additionalProperties` is dropped, and `type: ["integer", "null"]` becomes
`nullable: true`. The Zod schema therefore remains the single source of truth —
one definition still produces both the model-facing contract and the
validator.
